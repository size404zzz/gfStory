...
编辑器、模拟器和打包出的 viewer 里都有「录制」按钮，把「文本 → 可视化编辑」的成果直接变成视频方便传播：

- 点击「录制」后，在浏览器共享窗口里选择「此标签页」并勾选分享标签页音频，
  剧情就会自动从头播放：遇到选项停留片刻后自动选第一项，播完自动停止录制。
- 分辨率（1080p/720p/480p）、帧率（30/24/15）与码率（1~12Mbps）都可以在录制设置里机动调整。
- Chrome 126+ / Edge / Safari 会**直接录制 MP4**（系统硬件编码），播完即导出、零转码。
  其他浏览器（如 Firefox）录制 WebM，导出时再用内置 ffmpeg（约 31MB，首次从 CDN 加载并缓存）
  转码成 MP4——较慢且可随时取消退回 WebM。
- 录制期间按 Esc 可提前结束导出；录制时请保持标签页可见，浏览器会暂停后台标签页的渲染。
- 编辑器在「对白」步骤右上角有「录制视频」入口：把当前剧情编译进全屏播放器后再开始录制。

相关实现：采集与转码在 `src/story/recorder.ts`，录制流程与界面在
`src/components/viewer/StoryRecorder.vue`。
